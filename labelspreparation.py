...
class LabelsPreparation:
    def movingSlopeLabel(self, df, window=5, shift_=-5):
        label="trend"
        ...
        return ohlc_data[label]
        
    def weightedMovSlopeLabel(self, df, window_s=30, window_l=80, shift_s=-30, shift_l=-80, weight_s=0.5):
        label='trend'
        ohlc_data = df.copy(deep=True)
        ohlc_data['trend_s'] = self.movingSlopeLabel( ohlc_data, window=window_s, shift_=shift_s)
        ohlc_data['trend_l'] = self.movingSlopeLabel(ohlc_data, window=window_l, shift_=shift_l)
        ohlc_data[label]=(ohlc_data['trend_s']*weight_s)+(ohlc_data['trend_l']*(1-weight_s))
        return ohlc_data[label]
        
    def movingLabel(self, df,window=11, padding=0, shift=-1):
        label="trend"
        ohlc_data = df.copy(deep=True)
    ...
    def metaLabel(self, df, col1, col2):
        ret =  df[col1]*df[col2]
        ret = ret.replace("", "HOLD")
        return ret
